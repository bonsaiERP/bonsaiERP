...
describe Loans::ReceivePresenter do
  ...
  it '#new_ledger_in_path' do
    allow(view_context).to receive(:new_receive_loan_ledger_in).with(10).and_return("/loan_ledger_ins/10/new_receive")

    expect(loan_p.new_ledger_in_path).to eq("/loan_ledger_ins/10/new_receive")
  end

  it '#ledger_in_path' do
    allow(view_context).to receive(:receive_loan_ledger_in).with(10).and_return("/loan_ledger_ins/10/receive")

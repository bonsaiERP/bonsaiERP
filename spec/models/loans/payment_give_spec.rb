...
describe Loans::PaymentGive do
  ...
  context 'payment' do
    ...
    it "pays Loan" do
      ...
      lp = Loans::PaymentGive.new(attributes.merge(account_id: lf.loan.id))
      ...
      lp = Loans::PaymentGive.new(attributes.merge(account_id: lf.loan.id, amount: 60))
      ...
      lp = Loans::PaymentGive.new(attributes.merge(account_id: lf.loan.id, amount: 25, account_to_id: bank.id, exchange_rate: 2))
    ...
    it "pay with expense" do
      ...
      today = Date.today
      expense = Expense.new(total: 200, balance: 200, state: 'approved', currency: 'BOB', id: 100, contact_id: 1,
                         date: today, due_date: today)
      expense.stub(contact: build(:contact, id: 1))
      expense.save.should be_true

      lp = Loans::PaymentGive.new(attributes.merge(account_id: lf.loan.id, amount: 200, account_to_id: expense.id))

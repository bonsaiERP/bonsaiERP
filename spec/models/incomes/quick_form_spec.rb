...
describe Incomes::QuickForm do
  ...
  it "should present errors if the contact is wrong" do
    qi = Incomes::QuickForm.new(valid_attributes.merge(contact_id: 1000, account_to_id: 1200))
  ...
  context "Create income and check values" do
    before(:each) do
      ...
      AccountLedger.any_instance.stub(save: true, account_to: account_to, account: Income.new)
      Account.any_instance.stub(save: true)
    end

    it "creates a valid income" do
      qi = Incomes::QuickForm.new(valid_attributes)
    ...
    it "Can accept different values for conciliation when Bank account" do
      ...
      qi = Incomes::QuickForm.new(valid_attributes.merge(account_to_id: 3, verification: true))
      qi.create.should be_true
      ...
      # Other case
      qi = Incomes::QuickForm.new(valid_attributes.merge(account_to_id: 3, verification: false))
      qi.create.should be_true
    ...
    it "Assigns the currency of the account" do
      ...
      qi = Incomes::QuickForm.new(valid_attributes.merge(account_to_id: 3) )
